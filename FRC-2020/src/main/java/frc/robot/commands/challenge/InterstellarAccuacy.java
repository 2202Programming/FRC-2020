...
public class InterstellarAccuacy extends SequentialCommandGroup {
  ...
  public InterstellarAccuacy() {
    ...
    this.addCommands(leg1,leg2,leg3, leg4, leg5);
  ...
  Command build_leg(Pose2d startpose, Pose2d shootpose, ShooterSettings ss) {
    ...
    cmd.addCommands(
      new followTrajectory(drive, computeTrajectory(startpose, shootpose)),
      new ShooterWarmUp(ss)
     );
     cmd.andThen(new Shoot(ss))    // todo - add limelight compensator during shoot
